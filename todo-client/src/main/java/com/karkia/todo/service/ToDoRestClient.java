package com.karkia.todo.service;

import com.karkia.todo.config.ToDoRestClientProperties;
import com.karkia.todo.domain.ToDo;
import com.karkia.todo.error.ToDoErrorHandler;
...
/**
 * client that uses the RestTemplate class, which helps exchange information between this client and the server.
 * <p>
 * This class is using the RestTemplate class. RestTemplate is Spring’s central class for synchronous
 * client-side HTTP access. It simplifies communication with HTTP servers and enforces RESTful principles.
 * It handles HTTP connections, leaving application code to provide URLs (with possible template variables)
 * and extract results.
 * <p>
 * It contains all the actions that the ToDo app (Backend) has.
 */
@Service
